...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	//Computing the blurring for each element

	//Looping to extract each element
	for (int i = 0 ; i<h ; i++)
	{
		for (int j =0 ; j<w ; j++)
		{	
			...
			//Loop to apply the window for each element
			for (int dx = -1 ; dx <2 ; dx++)
			{
				for (int dy = -1 ; dy <2 ; dy++)
				{
					//Obtain the current multiplier (factor)
